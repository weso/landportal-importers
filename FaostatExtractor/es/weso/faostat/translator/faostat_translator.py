...
class FaostatTranslator(object):
    ...
    def __init__(self):
        ...
        self.land_area_reference = {}  # It will contain data with the general country areas
    ...
    def build_user_object(self, organization):
        user = User(user_id="FaostatImporter",
                    ip="156.35.90.35",
        ...
        if (not self.land_area_reference.has_key(key)):
            ...
            raise RuntimeError("Duplicated land area value for country {0} and \
                ...
                it looks like the data is corrupted" \
                               .format(candidate_register[TranslatorConst.COUNTRY],
                                       candidate_register[TranslatorConst.YEAR]))

    def key_for_land_area_ref(self, candidate_register):
        return str(candidate_register[TranslatorConst.COUNTRY_CODE]) + "_" + str(
            candidate_register[TranslatorConst.YEAR])
    ...
    def parse_register_fields(self, primitive_data):
        ...
        result.insert(TranslatorConst.COUNTRY_CODE,
                      self.parse_country_code(primitive_data[TranslatorConst.COUNTRY_CODE]))
        ...
        result.insert(TranslatorConst.ELEMENT_GROUP,
                      self.parse_element_group(primitive_data[TranslatorConst.ELEMENT_GROUP]))
        result.insert(TranslatorConst.ELEMENT_CODE,
                      self.parse_element_code(primitive_data[TranslatorConst.ELEMENT_CODE]))
